...
export function ChatWidget({
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 md:bottom-6 md:right-6 md:w-auto md:left-auto">
            {/* Chat Window */}
            {(isOpen && !isMinimized) && (
                <div className="fixed inset-x-3 bottom-20 top-4 z-[60] w-auto bg-background-secondary border border-border-primary rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300 md:inset-auto md:right-6 md:bottom-28 md:h-[min(600px,calc(100vh-8rem))] md:w-[min(380px,calc(100vw-3rem))] lg:right-56 lg:bottom-6">
                    {/* Header */}
